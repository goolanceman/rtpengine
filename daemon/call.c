...
retry:
		...
		case XF_KAMAILIO:
			g_string_append_printf(body, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
					...
					"</params>"
					"</methodCall>",
					STR_FMT(tag), STR_FMT(tag2), STR_FMT(tag3));
			break;
		}
...
next_k:
	...
	/* Single call-level NOTICE SUMMARY for LOG_NOTICE (level 5) operators */
	{
		...
		for (__auto_type l = c->monologues.head; l; l = l->next) {
			...
			for (unsigned int m = 0; m < ml->medias->len; m++) {
				...
				if (!md)
					continue;
				medias_n++;
				/* Prefer codec that saw packets; else list offered SDP codecs */
				if (proto_is_rtp(md->protocol)) {
					rtp_pt = __rtp_stats_codec(md);
					if (rtp_pt && rtp_pt->encoding_with_params.len) {
						...
						for (__auto_type cl = md->codecs.codec_prefs.head; cl; cl = cl->next) {
							const rtp_payload_type *pt = cl->data;
				...
				for (__auto_type o = md->streams.head; o; o = o->next) {
					...
					if (PS_ISSET(ps, FALLBACK_RTCP))
						continue;
					ports_open++;
					uint64_t pin = atomic64_get_na(&ps->stats_in->packets);
					uint64_t pout = atomic64_get_na(&ps->stats_out->packets);
					tot_in_p += pin;
					tot_in_b += atomic64_get_na(&ps->stats_in->bytes);
					tot_in_e += atomic64_get_na(&ps->stats_in->errors);
					tot_out_p += pout;
					tot_out_b += atomic64_get_na(&ps->stats_out->bytes);
					tot_out_e += atomic64_get_na(&ps->stats_out->errors);
		...
		/* Also reprint per-port lines at NOTICE so level-5 sees them */
		for (__auto_type l = c->monologues.head; l; l = l->next) {
			ml = l->data;
			for (unsigned int m = 0; m < ml->medias->len; m++) {
				md = ml->medias->pdata[m];
				...
				if (proto_is_rtp(md->protocol)) {
					...
					if (rtp_pt && rtp_pt->encoding_with_params.len)
						...
						const rtp_payload_type *pt0 = md->codecs.codec_prefs.head->data;
				...
				for (__auto_type o = md->streams.head; o; o = o->next) {
					...
					uint64_t pin = atomic64_get_na(&ps->stats_in->packets);
					uint64_t pout = atomic64_get_na(&ps->stats_out->packets);
					int idle = (int) (rtpe_now.tv_sec - packet_stream_last_packet(ps));
					ilog(LOG_NOTICE,
						...
						"  idle_sec=%d"
						"  | Opened media port stats",
						STR_FMT_M(&c->callid),
						STR_FMT_M(&ml->tag),
						md->index, STR_FMT(&md->type),
						codec_s,
						md->protocol ? md->protocol->name : "(unknown)",
						sockaddr_print_buf(&lep->address),
						(unsigned int) lep->port,
						raddr, (unsigned int) ps->endpoint.port,
						(!PS_ISSET(ps, RTP) && PS_ISSET(ps, RTCP)) ? " (RTCP)" : "",
						pin, atomic64_get_na(&ps->stats_in->bytes),
						pout, atomic64_get_na(&ps->stats_out->bytes),
						atomic64_get_na(&ps->stats_in->errors),
						atomic64_get_na(&ps->stats_out->errors),
						idle);
					if (PS_ISSET(ps, RTP) && pin == 0 && pout == 0)
