#!/usr/bin/env bash
# Assemble rtpengine-11.5.1.24-rich-logs-<UTC datetime>.tar.gz from the
...
REL="${ROOT}/release-bins/11.5.1.24-rich-logs"
BIN_ROOT="${ROOT}/build-tools/bins"
TAR_DIR="${ROOT}/build-tools/tars"
DEB="${BIN_ROOT}/debian"
RHEL="${BIN_ROOT}/rhel"
NAME="rtpengine-11.5.1.24-rich-logs"
...
install -m 0644 "${INNER}/debian-bins-package.tar.gz" \
  "${BASE}/release-bins/11.5.1.24-rich-logs/debian-bins-package.tar.gz"
for p in debian rhel; do
  mkdir -p "${BASE}/release-bins/11.5.1.24-rich-logs/${p}"
  install -m 0755 "${BIN_ROOT}/${p}/rtpengine" \
    "${BASE}/release-bins/11.5.1.24-rich-logs/${p}/rtpengine"
  install -m 0755 "${BIN_ROOT}/${p}/rtpengine-recording" \
    "${BASE}/release-bins/11.5.1.24-rich-logs/${p}/rtpengine-recording"
done
# never ship local backups
rm -rf "${BASE}/release-bins/11.5.1.24-rich-logs/"*.bak.* \
       "${BASE}/release-bins/11.5.1.24-rich-logs/"*.bak
...
cp -a "${SCRIPT_DIR}/CONFLUENCE-11.5.1.24-rich-logs-usage.md" "${BASE}/build-tools/"
cp -a "${SCRIPT_DIR}/BINS.md" "${BASE}/build-tools/"
printf '%s\n' "11.5.1.24" > "${BASE}/VERSION"
printf '%s\n' "${SHA}" > "${BASE}/GIT_SHA"
...
for p in debian rhel; do
  echo "-- ${p} recording --"
  tmp="$(mktemp)"
  tar -xOf "${OUT}" "${NAME}/release-bins/11.5.1.24-rich-logs/${p}/rtpengine-recording" > "${tmp}"
