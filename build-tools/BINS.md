# Canonical binary directories (11.5.1.24 rich-logs)

Use **one directory per OS**. Do not keep extra copies (`rhel-binaries/`,
`rhel-binaries-11.5.1.24/`, `rhel/bins/`, `debian-bins/bins/` for deploy).

| OS | Directory | Binaries |
|----|-----------|----------|
| Debian 13 (siprec) | `release-bins/11.5.1.24-rich-logs/debian/` | `rtpengine`, `rtpengine-recording` |
| RHEL 8 / EL8 (lab) | `release-bins/11.5.1.24-rich-logs/rhel/` | `rtpengine`, `rtpengine-recording` |
...
3. Docker extract:
   - Debian: `OUT_DIR=release-bins/11.5.1.24-rich-logs/debian ./build-tools/docker-build-binaries.sh`
...
OUT=/tmp/rtpengine-11.5.1.24-rich-logs.tar.gz ./build-tools/package-rich-logs-tarball.sh
```

Output: `rtpengine-11.5.1.24-rich-logs-<UTC YYYYMMDDHHMMSS>.tar.gz`.
