#!/bin/bash
# 11.5.1.24 rich-log USERSPACE promote/rollback for Debian siprec.
...
# Resolve path of running binary via systemd MainPID + /proc/PID/exe
# (pgrep -x fails: "rtpengine-recording" is truncated to "rtpengine-recor")
running_bin_path() {
  local name="$1"
  local pid="" path="" u
  local -a units=()
  case "$name" in
    rtpengine)
      units=(rtpengine.service ngcp-rtpengine-daemon.service)
      ;;
    rtpengine-recording)
      units=(rtpengine-recording.service ngcp-rtpengine-recording-daemon.service)
      ;;
    *)
      units=()
      ;;
  ...
  for u in "${units[@]}"; do
    ...
    if [[ -n "${pid:-}" && "$pid" != "0" && -d "/proc/$pid" ]]; then
      ...
      # exe may show " (deleted)" if replaced underfoot — strip that
      path=${path% (deleted)}
  ...
  # fallback: scan /proc for cmdline containing the binary name
  local d cmd
  for d in /proc/[0-9]*; do
    pid=${d#/proc/}
    cmd=$(tr '' ' ' <"$d/cmdline" 2>/dev/null || true)
    case "$name" in
      rtpengine)
        [[ "$cmd" == *rtpengine* && "$cmd" != *rtpengine-recording* ]] || continue
        ;;
      rtpengine-recording)
        [[ "$cmd" == *rtpengine-recording* ]] || continue
        ;;
    esac
    path=$(readlink -f "$d/exe" 2>/dev/null || true)
    path=${path% (deleted)}
    if [[ -n "$path" && -e "$path" ]]; then
      echo "$path"
      return 0
...
Bins resolution for promote (first match):
  1) BIN_DIR=... env
  2) $ROOT/bins  (packaged tarball layout)
  3) $REPO/release-bins/11.5.1.24-rich-logs/{debian|rhel}  (OS-matched)

See build-tools/BINS.md — one dir per OS. Do not use rhel-binaries/ or debian-bins/bins.

Examples:
  sudo $0 status
  BIN_DIR=\$PWD/release-bins/11.5.1.24-rich-logs/debian sudo -E $0 promote
  BIN_DIR=\$PWD/release-bins/11.5.1.24-rich-logs/rhel   sudo -E $0 promote
  sudo $0 list-backups
...
resolve_bin_dir() {
  ...
  if [[ -n "${BIN_DIR:-}" ]]; then
    ...
    BIN_DIR="${ROOT}/bins"
  elif [[ -x "${REPO}/release-bins/11.5.1.24-rich-logs/${os}/rtpengine" ]]; then
    BIN_DIR="${REPO}/release-bins/11.5.1.24-rich-logs/${os}"
  else
    die "no binaries found. Set BIN_DIR= to release-bins/11.5.1.24-rich-logs/{debian|rhel}"
  fi
  [[ -x "${BIN_DIR}/rtpengine" ]] || die "missing ${BIN_DIR}/rtpengine"
  [[ -x "${BIN_DIR}/rtpengine-recording" ]] || die "missing ${BIN_DIR}/rtpengine-recording"
  echo "==> BIN_DIR=${BIN_DIR} (os=${os})"
}
...
cmd_promote() {
  ...
  install_one() {
    ...
    # Never treat a directory (e.g. repo ./rtpengine) as an install target.
    if [[ -d "$dest" || "$dest" != /* ]]; then
      echo "  skip $dest (not an absolute regular-file path)"
      return 0
    fi
  ...
  echo "==> kernel module (unchanged — expect xt_RTPENGINE 11.5)"
  ...
  ls -la /proc/rtpengine/control || die "/proc/rtpengine/control missing — load 11.5 kernel package first"
  ...
  sleep 1
  local rp rr
  rp=$(running_bin_path rtpengine)
  rr=$(running_bin_path rtpengine-recording)
