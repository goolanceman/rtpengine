# rtpengine 11.5.1.24 Rich Recording Logs — Deployment Guide (Debian siprec)

**Audience:** SIPREC / media operators  
**Branch:** `rich-recording-logs-11.5.1.24`  
...
On the **siprec host**:
    ...
    cd ~/rtpengine
    git fetch origin
    git checkout rich-recording-logs-11.5.1.24
    git pull origin rich-recording-logs-11.5.1.24

Confirm release binaries exist (canonical Debian dir only — see `build-tools/BINS.md`):

    ls -la release-bins/11.5.1.24-rich-logs/debian/
    # expect: rtpengine  rtpengine-recording

    file release-bins/11.5.1.24-rich-logs/debian/rtpengine
    file release-bins/11.5.1.24-rich-logs/debian/rtpengine-recording
...
Rebuild the deploy tarball after replacing bins (filename always ends with UTC datetime):
    ...
    # → rtpengine-11.5.1.24-rich-logs-YYYYMMDDHHMMSS.tar.gz

Confirm production is 11.5 + xt_RTPENGINE (typical):
...
3. OS-matched `release-bins/11.5.1.24-rich-logs/{debian|rhel}`
...
### 3.5 Verify after promote
    ...
    file /usr/bin/rtpengine-recording \
         release-bins/11.5.1.24-rich-logs/debian/rtpengine-recording
...
| Spool | /var/spool/recording | /var/spool/recording-test-11.5 |
| WAV out | /tmp/recordings | /tmp/recordings-test-11.5 |
...
|------|------|
| Branch | rich-recording-logs-11.5.1.24 |
| **Debian bins (only)** | `release-bins/11.5.1.24-rich-logs/debian/` |
| **RHEL bins (only)** | `release-bins/11.5.1.24-rich-logs/rhel/` |
